...
/***/ function(module, exports, __webpack_require__) {
	...
	function Folders () {
	  ...
	  var navContainer = $('.main-nav-wrapper');
	  var folderToggle = $(this).find('.folder-toggle');

	  var getOtherOpenFolder = function (folder) {
	    return folder.parent().siblings().find('.folder-open');
	  };
	   ...
	   * @return {Boolean}
	   */
	  var checkFolderOverflow = function () {
	    // we don't need to add a scrolling class if the entire nav is already scrollable
	    var navAlreadyOverflowing = navContainer.hasClass('scroll');
	    if (navAlreadyOverflowing) {
	      return false;
	    }
	  
	    var folderOverflowing = subnav[0].getBoundingClientRect().bottom > navContainer[0].getBoundingClientRect().bottom;
	   ...
	   *
	   */
	  var setFolderState = function (e) {
	    var currentlyOpenFolder = getOtherOpenFolder($(e.target));
	    if (currentlyOpenFolder) {
	      currentlyOpenFolder.removeClass('folder-open');
	    }
	  
	    $(e.target).toggleClass('folder-open');
	    navContainer.removeClass('scroll-folder');

	    if ($(e.target).hasClass('folder-open') && checkFolderOverflow()) {
	      navContainer.addClass('scroll-folder');
	    }
	  };

	  folderToggle.on('click', setFolderState);
	}

	controller.registerController('Folders', Folders);
